...
function App() {
  ...
  React.useEffect(() => {
    ...
    document.querySelectorAll(".holo-stage").forEach(s => {
      s.classList.toggle("show-model", t.avatarShader === "stock 3D");
    });
    const cap = document.getElementById("holo-cap");
    if (cap) cap.textContent = (t.avatarShader === "stock 3D") ? "stock model · drag to rotate" : "projection · aravind.v";
  ...
  return (
    <TweaksPanel>
      ...
      <TweakRadio label="Figure" value={t.avatarShader}
        options={["you", "stock 3D"]}
        onChange={(v) => setTweak("avatarShader", v)} />
